...
establish with a rule fixed before the stimulus existed.

\textbf{The two findings come apart, and that is the paper's contribution.} The compositional
dissociation is significant on every model we could measure it on, \emph{including the one model
where the anchoring objective reverses}; the objective's benefit is conditional on the model in a
way the dissociation is not. Adapting to obfuscated code buys recombination of what the training
data covered, and anchoring recovers what breadth costs --- neither buys invariance.
\end{abstract}
...
\begin{table}[t]\centering\small
\caption{The divergence result across models, at the critical level. All eight models are read.}
\label{tab:divmodels}
