...
## Master and setup tables (added 2026-09-13)

Generated by `python scripts/analysis/59_master_tables.py`, which also writes `docs/MASTER_TABLES.md`
from the same cells. Every cell carries three numbers: raw accuracy, change in points against the
untuned model on the same condition, and accuracy as a percentage of the untuned model's clean-code
accuracy in the same direction. A cell whose format-failure rate exceeds 0.25 renders as `fmt` and
enters no mean; the gate is per cell, stricter than the pooled registration.

| file | where | `\input` line |
|---|---|---|
| `setup_models.tex` | Setup › Models | `\input{tables/setup_models}` |
| `setup_dataset.tex` | Setup › Dataset | `\input{tables/setup_dataset}` |
| `setup_taxonomy.tex` | Setup › Obfuscation taxonomy (single transforms) | `\input{tables/setup_taxonomy}` |
