...
them is worth two to four points. Outside them the ordering inverts.
...
specialists gives up two to four points on seen compositions, where
...
return value, produce a call that yields it). On seen compositions the adaptations are within two to
four points of one another; on the other two the ordering inverts. Reporting only the first regime,
...
single adapter trained on all five transformations is below every arm assembled from per-transformation
specialists on the unseen family, on all eight models --- and below an adapter trained on unobfuscated
...
significantly below it in any regime on any model, at a cost of two to four forward points where
