...
clean-code control and a KL-consistency objective that ties the
...
we tie learning to something that is invariant across all such
...
code. Instead, it
changes the learning target itself: rather than tying adaptation to
the surface patterns of individual transformations, it ties the
...
the unseen family it is competitive with, not superior to, the router system
...
\textbf{2. Adaptation on obfuscated code exceeds clean-code accuracy, not merely recovers it.} Measured against the untuned model's accuracy on unobfuscated source --- the performance obfuscation removes --- adapted models reading obfuscated programs reach 110--195\,\% of it, above 100\,\% on 93\,\% of model\,$\times$\,condition-group cells, including on a held-out obfuscator family. The recovery is direction-specific: it does not extend to the untrained direction, which is what the rest of the paper is about.

\textbf{3. Monolithic training is the worst use of the data outside the training distribution.} A
...
\textbf{4. Adaptation locks the direction it was trained in, and we measure it directly.} Every
...
\textbf{5. Weight-space merging as the arm that never breaks, with its price stated.} Merging the same
