...
reverse direction. The first finding is the size of what adaptation buys. Obfuscation costs an untuned model up to
half its clean-code accuracy; an adapted model reading obfuscated programs reaches 110--195\,\% of
the untuned model's \emph{clean-code} accuracy, above 100 on 93\,\% of model\,$\times$\,condition-group
cells and on an obfuscator family it never saw. Adaptation is not tolerating the transform; it is
teaching something the model lacked on unobfuscated source too. On seen compositions every
adaptation does this and the choice between them is worth a few points. Outside them the ordering inverts. Training one adapter on everything --- the obvious use
