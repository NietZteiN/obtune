...
That the two views are not redundant is shown directly by the backward-trained control
(Section~\ref{sec:results}): a model trained on input prediction improves at \emph{both} tasks, while
models trained on output prediction improve at one and lose the other. A single view cannot
distinguish those two outcomes; two views separate them immediately.
...
rather than in task fit. Stated as \emph{never significantly below} the untuned model rather than strictly above it --- the
form the data supports --- exactly one arm in this study passes on all eight models, the DARE-TIES
merge (Section~\ref{sec:results}), and it does so without being the best arm in any single view. Two properties of this criterion are worth stating. It is conservative in the
...
and fixed, the KL-consistency arm and the router system are the strongest forward arms and the three to six
...
and it costs one adapter and no inference-time machinery. The router system matches or beats it on the unseen
