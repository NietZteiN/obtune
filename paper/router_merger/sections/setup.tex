...
% >>> CONTRADICTED BY MEASUREMENT, 2026-09-14. The next sentence says a gated cell "measures the
% prompt contract rather than the task". scripts/analysis/67_backward_failure_modes.py decomposes
% those cells and that is false of many of them: on CodeGemma's anchored arm 96 %% of the failures
% are replies that are exactly the gold FORWARD answer, and on StarCoder2's tuned arms 79-91 %%.
% Those cells measure forward-locking, which is a result. Others are genuinely malformed, and a
% few are truncation at the 128-token cap. Left unedited because the fix interacts with the
% framing decision; log/transfer/2026-09-14_forward-locking-is-the-mechanism.md has the numbers
% and tables/master_collapse.tex is the table, generated and not yet \input anywhere. <<<
A cell
