...
## Hypotheses — open
- **H-L1b-L0-trade** — breadth training's L0 cost and L1b gain are the same items ("distrust identifiers"). CONFIRM if `mono_all`'s L1b gains concentrate on programs where it loses L0 relative to `tuned_L0`; REFUTE if independent. Existing cells, no GPU.
