...
| forgetting (held out) | 7B | **MBPP+ pass@1, 399 tasks**, 7 arms | — | `results/forgetting/mbppplus_qwen25c-7b_7b_*.json` |
| `e9_cleanneg_qwen1.5b` | 1.5B | **paper-literal negatives**: `base`/`sft`/`cft`/`cftclean` in ONE pass | 12,000 | `results/2026-08-18_cft-bidirectional/qwen25c-1.5b/python/e9_cleanneg_qwen1.5b/` |
...
| §8 Table 5, HumanEval+ cols | HumanEval+ `plus` split | `results/forgetting/humanevalplus_{qwen25c-1.5b_*,qwen25c-7b_7b_*}.json`, key `pass@1_plus` |
| §8 Table 5, **MBPP+ col** | MBPP+ `plus` split, 399 tasks | `results/forgetting/mbppplus_qwen25c-7b_7b_*.json`, key `pass@1_plus`. Each carries a `provenance` block (git commit, script sha256, adapter sha256, seed, GPU) and a 399-entry `per_task` dict |
| §8 contrasts and CIs | paired bootstrap over the 399 MBPP+ tasks, 10,000 resamples, seed 17; sign confirmed by exact McNemar on discordant pairs | computed from the `per_task` dicts above. `sft`−`base` −5.3 [−9.0,−1.5] p=8.6e−3; `mix50`−`sft` −0.3 [−4.0,+3.5] p=1.00; both−one −1.2 [−3.8,+1.5]; pooled trained−base −5.1 [−8.0,−2.1] |
| §8 "74 of the 164" | HumanEval programs in the **train split** | `data/train/base/python.jsonl` (104 rows with `source=humaneval`) joined to `data/splits/python.json` → 74 train / 25 test / 5 val. Was stated as 104 through draft v2, which counted the corpus rather than the split |
| App. C `cftclean` | strict reverse, 1.5B, paper-literal negatives | `.../e9_cleanneg_qwen1.5b/summary.json`, `reverse_success_strict` for `system=cftclean`, `condition=ALL`, `direction=reverse` = 0.004667 → 0.5 %. Same pass: `cft` 0.002, `sft` 0.002, `base` 0.028 |
| App. C contrasts | paired cluster bootstrap by `program_id`, 2,000 resamples, seed 17 | `.../e9_cleanneg_qwen1.5b/contrasts.md`, written by `scripts/srh/24_contrasts.py`. `cftclean`−`cft` +0.3 [−0.1,+0.7]; `cftclean`−`sft` +0.3 [−0.1,+0.7]; `cftclean`−`base` −2.3 [−3.3,−1.4] |
| App. C pool sizes | `cleanneg` pool is LARGER than the published one | `data/train/cft/python__cleanneg/pool_report.json` — 6,058 pos/neg against 5,611. A sibling directory; the published pools in `data/train/cft/python/` are untouched (mtimes still 2026-08-08) |
| App. C training loss | `cftclean` 0.3537 vs `cft` 0.3544 | `runs/adapters_cft/qwen25c-1.5b/python/{cftclean,cft}_r32_s17/training_summary.json`, key `train_loss` |
| §8 "0 of 399" | MBPP absent from the corpus | `data/manifests/corpus_python.json` records `tiers: ['tier1']`; MBPP/MBPPPlus are declared only under `tier2` in `configs/sources.yaml` and were never built. Provenance histogram over `data/train/base/python.jsonl`: apps 1584 / cruxeval 543 / humaneval 104 / **mbpp 0** |
...
| ~~HumanEval+ for the dose arms~~ | **RUN 2026-08-17.** Now Appendix E. The retained capability is not monotone in reverse share, so no dose-response conclusion is drawn on that axis at 1.5B. |
| ~~HumanEval+ for 7B `rev` / `fwd2x`~~ | **RUN 2026-08-17.** `rev` .799, `fwd2x` .805. Table 5's 7B column is complete. |
...
- Not "our deviation on negatives explains the null." As of draft v3 the paper-literal
  construction was **built and run** (App. C): it reaches 0.5 % against `cft`'s 0.2 %, a
  difference whose interval spans zero, and sits 2.3 pp below the untouched model. The theoretical
  argument is retained, but it is no longer the only thing carrying that point.
- Not "the flip is free" without qualification. Free is scoped to training budget throughout.
  As of draft v3 the general-ability cost is stated against the **forward-only baseline**, where
  it is nil (`mix50`−`sft` = −0.3 pp [−4.0,+3.5] on MBPP+), and against the **untouched model**,
  where every trained arm loses about 5 points regardless of mixture. The draft-v2 claim that
  bidirectional arms specifically cost 6–7 points was a HumanEval+ contamination artifact and
  does not survive the held-out probe.
...
- **Not "every auxiliary-task result on paired data is a directional artifact."** Draft v4 makes
  the general claim explicit, so the boundary on it matters more than before. The conclusion
  states the limit in the paper's own words: the confound is *real, cheap to test for, and
  currently untested*. We measured one instance. Anything stronger is not supported and is the
  first thing a reviewer will press.
- **Not "direction explains the general-ability cost."** It does not. On the held-out probe every
  trained arm loses ~5 pts regardless of direction mix (`mix50`−`sft` = −0.3 pp [−4.0,+3.5]).
  The confound claim is about the *reverse-capability* effect only.
- **Not "the objective is contrastive in the embedding sense."** Appendix B's own argument is that
  the published description does not determine this, which is why the title says "Direction
  Confounds Attribution" rather than naming the objective family.
